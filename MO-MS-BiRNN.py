...
def _MLP_RNN(n_inputs, n_outputs, R_layers_i, R_layers_f, rnn_hidden_units_i_, rnn_hidden_units_f_, mlp_hidden_units_, dropout):
    ...
    _hidden_f_backward = _get_features_input(_inputs, forward = False)
    # Intput Recurrent Layers
    _hidden_i          = __input_layer(_hidden_i, rnn_hidden_units_i_, R_layers_i, dropout)
    _hidden_f_forward  = __input_layer(_hidden_f_forward, rnn_hidden_units_f_, R_layers_f, dropout)
    _hidden_f_backward = __input_layer(_hidden_f_backward, rnn_hidden_units_f_, R_layers_f, dropout)
    # Define The Input Layers for each feature source
    _hidden = keras.layers.concatenate([_hidden_i, _hidden_a, _hidden_f_forward, _hidden_f_backward], axis = 1)
...
def _model_training(X_tr_, Y_tr_, X_ts_, Y_ts_, theta_, i_R_lay, f_R_lay, i_M_lay, R_lay_i, R_lay_f, i_R_neurons, path):
    ...
    model_name = path + r'MO-MS-BiRNN_v31-1_{}{}{}{}{}{}.h5'.format(i_cov, i_R_lay, f_R_lay, i_M_lay, R_lay_i, R_lay_f)
...
def _kfold_cross_validation(theta_, args_):
    ...
    X_tr_, Y_tr_, R_layers_i, R_layers_f, M_layers, R_layer_i, R_lay_f, path = args_
    ...
    for idx_tr_, idx_ts_ in KFold(n_splits     = 3,
        ...
        # Traning Model
        error = _model_training(X_val_tr_, Y_val_tr_, X_val_ts_, Y_val_ts_, theta_, R_layers_i, R_layers_f, M_layers, R_layer_i, R_lay_f, i_R_neurons = 3.5036535952058725,
                                                                                                                                          path        = path)[0].mean()
        error_.append(error)
...
def _get_BO_MO_MLP_RNN_cross_validation(X_tr_, Y_tr_, i_R_lay, f_R_lay, i_M_lay, R_lay_i, R_lay_f, path):
    ...
    return _BO(_kfold_cross_validation, bounds_      = bounds_, 
                                        ...
                                        xi           = 1.,
                                        kappa        = 10.,
                                        X_0_         = _random_init(_kfold_cross_validation, bounds_, args_, n_init = 45),
                                        n_iterations = 65,
                                        maximize     = True,
                                        args_        = args_,
                                        n_restarts   = 10,
...
if validation:
    ...
    # Dump data in a .csv
    name = r'/users/terren/solar_forecasting/logs/neural_networks/MO-MS-BiRNN_v31-1.csv'
    ...
    # 0.1606569104642009 207.51687049865725 [8. 2. 2. 3. 0. 2.] [-4.128848871853361, 6.797257650174725, 2.298402893509494, 2.7655798505187073, -5.647225113918165]
    theta_  = [-4.128848871853361, 6.797257650174725, 2.298402893509494, 2.7655798505187073, -5.647225113918165]
    i_cov   = 8
    i_R_lay = 2
    f_R_lay = 2
    i_M_lay = 3
    ...
    np.savetxt(r'/users/terren/solar_forecasting/model/deep_learning/test_{}_MO-MS-BiRNN_prediction.csv'.format(i_init), Y_ts_hat_, delimiter = ',')

    x_ = [i_init, time_[0], time_[1], np.mean(error_ts_)] + error_ts_.tolist()

    name = r'/users/terren/solar_forecasting/logs/neural_networks/test_MO-MS-BiRNN_v31-1.csv'
    with open(name, 'a', newline = '\n') as f:
        writer = csv.writer(f)
        writer.writerow(x_)
