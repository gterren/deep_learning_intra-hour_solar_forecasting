...
def _MLP_RNN(n_inputs, n_outputs, R_layers_i, R_layers_f, rnn_hidden_units_i_, rnn_hidden_units_f_, mlp_hidden_units_, dropout):
    ...
    print(_hidden_i.shape, _hidden_a.shape, _hidden_f_forward.shape)
    # Define The Input Layers for each feature source
    _hidden = keras.layers.concatenate([_hidden_i, _hidden_a, _hidden_f_forward], axis = 1)
...
def _model_training(X_tr_, Y_tr_, X_ts_, Y_ts_, theta_, i_R_lay, f_R_lay, i_M_lay, R_lay_i, R_lay_f, i_R_neurons, path):
    ...
    model_name = path + r'MO-MS-RNN-B_v31-1_{}{}{}{}{}{}.h5'.format(i_cov, i_R_lay, f_R_lay, i_M_lay, R_lay_i, R_lay_f)
...
def _get_BO_MO_MLP_RNN_cross_validation(X_tr_, Y_tr_, i_R_lay, f_R_lay, i_M_lay, R_lay_i, R_lay_f, path):
    ...
    return _BO(_kfold_cross_validation, bounds_ = bounds_, _aqf = 'EI', xi = 1., kappa = 10.,
               X_0_ = _random_init(_kfold_cross_validation, bounds_, args_, n_init = 50),
               n_iterations = 75, maximize = True, args_ = args_, n_restarts = 10, display = True)
...
if validation:
    ...
    # Dump data in a .csv
    name = r'/users/terren/solar_forecasting/logs/neural_networks/MO-MS-RNN_v31-1.csv'
    ...
    # 0.1589579836676854 394.90296483039856 [6. 2. 2. 2. 0. 0.] [-5.498476614958468, 5.168638849144871, 1.1399292503773704, 1.7304598975179386, -9.060573858342496]
    theta_  = [-5.498476614958468, 5.168638849144871, 1.1399292503773704, 1.7304598975179386, -9.060573858342496]
    i_cov   = 6
    i_R_lay = 2
    f_R_lay = 2
    i_M_lay = 2
    ...
    np.savetxt(r'/users/terren/solar_forecasting/model/deep_learning/test_{}_MO-MS-RNN_prediction.csv'.format(i_init), Y_ts_hat_, delimiter = ',')

    x_ = [i_init, time_[0], time_[1], np.mean(error_ts_)] + error_ts_.tolist()

    name = r'/users/terren/solar_forecasting/logs/neural_networks/test_MO-MS-RNN_v31-1.csv'
    with open(name, 'a', newline = '\n') as f:
        writer = csv.writer(f)
        writer.writerow(x_)
