...
def _MLP_RNN(n_inputs, n_outputs, R_layers_i, rnn_hidden_units_, mlp_hidden_units_, dropout):
    ...
    def _get_concatenated_input(_inputs):

        if len(index_) > 1:
            _hidden_1 = keras.layers.Lambda(__feature_vector, arguments = ({'idx_': index_[1]}))(_inputs)
            _hidden   = _hidden_1
        if len(index_) > 2:
            _hidden_2 = keras.layers.Lambda(__feature_vector, arguments = ({'idx_': index_[2]}))(_inputs)
            _hidden   = keras.layers.concatenate([_hidden_1, _hidden_2], axis = 1)

        if len(index_) > 3:
            _hidden_3 = keras.layers.Lambda(__feature_vector, arguments = ({'idx_': index_[3]}))(_inputs)
            _hidden   = keras.layers.concatenate([_hidden_1, _hidden_2, _hidden_3], axis = 1)

        if len(index_) > 4:
            _hidden_4 = keras.layers.Lambda(__feature_vector, arguments = ({'idx_': index_[4]}))(_inputs)
            _hidden   = keras.layers.concatenate([_hidden_1, _hidden_2, _hidden_3, _hidden_4], axis = 1)

        if len(index_) > 5:
            _hidden_5 = keras.layers.Lambda(__feature_vector, arguments = ({'idx_': index_[5]}))(_inputs)
            _hidden   = keras.layers.concatenate([_hidden_1, _hidden_2, _hidden_3,
                                                  _hidden_4, _hidden_5], axis = 1)
        if len(index_) > 6:
            _hidden_6 = keras.layers.Lambda(__feature_vector, arguments = ({'idx_': index_[6]}))(_inputs)
            _hidden   = keras.layers.concatenate([_hidden_1, _hidden_2, _hidden_3,
                                                  _hidden_4, _hidden_5, _hidden_6], axis = 1)
        if len(index_) > 7:
            _hidden_7 = keras.layers.Lambda(__feature_vector, arguments = ({'idx_': index_[7]}))(_inputs)
            _hidden   = keras.layers.concatenate([_hidden_1, _hidden_2, _hidden_3,
                                                  _hidden_4, _hidden_5, _hidden_6, _hidden_7], axis = 1)
        return _hidden
    # Defien inputs
    ...
    _hidden_i = __input_layer(_hidden_i, rnn_hidden_units_, R_layers_i, dropout)
    # Define The Input Layers for each feature source
    if len(index_) > 1:
        _hidden_f = _get_concatenated_input(_inputs)
        _hidden = keras.layers.concatenate([_hidden_i, _hidden_f], axis = 1)
    else:
        _hidden = _hidden_i
...
def _model_training(X_tr_, Y_tr_, X_ts_, Y_ts_, theta_, i_R_lay, i_M_lay, R_lay_i, path):
    print(i_R_lay, i_M_lay, R_lay_i, np.exp(theta_[0]), int(np.exp(theta_[1])), np.exp(theta_[2]), np.exp(theta_[3]), np.exp(theta_[4]))
    model_name = path + r'MO-RNN_v2_v31-1_{}{}{}{}.h5'.format(i_cov, i_R_lay, i_M_lay, R_lay_i)
    ...
    rnn_hidden_units_ = _define_rnn_architecture(R_layers = i_R_lay, R_neurons = np.exp(theta_[2]))
    print(rnn_hidden_units_)
    ...
    # Defime RNN model
    t_tr   = time.time()
    _model = _MLP_RNN(n_inputs, n_outputs, R_lay_i, rnn_hidden_units_, mlp_hidden_units_, dropout = np.exp(theta_[4]))
    # Compile Model
...
def _kfold_cross_validation(theta_, args_):
    ...
    X_tr_, Y_tr_, R_layers, M_layers, R_layer_i, path = args_
    ...
    for idx_tr_, idx_ts_ in KFold(n_splits     = 3,
        ...
        error = _model_training(X_val_tr_, Y_val_tr_, X_val_ts_, Y_val_ts_, theta_,
                                R_layers, M_layers, R_layer_i, path)[0].mean()
        error_.append(error)
    return np.mean(error_)

# BO-MO-MLP-RNN K-Fold Cross-Validation of the model Parameters
def _get_BO_MO_MLP_RNN_cross_validation(X_tr_, Y_tr_, i_R_lay, i_M_lay, R_lay_i, path):
    ...
    M_neurons_     = (1.5, 5.)
    dropout_       = (-10,-2.)
    bounds_ = _BO_bounds(theta_ = [learning_rate_, batch_size_, R_neurons_, M_neurons_, dropout_])
    # [-5.939030243564477, 5.915782749094877, 3.5036535952058725, 1.813729223390288, -8.104985536762872]
    # Constants Initialization
    args_     = (X_tr_, Y_tr_, i_R_lay, i_M_lay, R_lay_i, path)
...
if validation:
    ...
    i_R_lay = int(sys.argv[2])
    i_M_lay = int(sys.argv[3])
    R_lay_i = int(sys.argv[4])
    i_structure = 0
    # Get Experiment for the i-th Job
    print(i_cov, i_R_lay, i_M_lay, R_lay_i, i_structure)
    ...
    theta_, error_val_ = _get_BO_MO_MLP_RNN_cross_validation(X_tr_, Y_tr_, i_R_lay, i_M_lay, R_lay_i, path = r'/users/terren/solar_forecasting/model/deep_learning/')
    print(theta_, -error_val_)
    # Training Optimal MO-MLP-RNNS Architecture Parameters
    tm1       = time.time()
    error_ts_ = _model_training(X_tr_, Y_tr_, X_ts_, Y_ts_, theta_, i_R_lay, i_M_lay, R_lay_i, path = r'/users/terren/solar_forecasting/model/deep_learning/')[0]
    tm        = time.time() - tm1
    print(tm, error_ts_, error_ts_.mean())
    # Save Results by row in a .csv file
    x_ = [[i_cov, i_R_lay, i_M_lay, R_lay_i]] + [np.stack(theta_).tolist()] + np.stack(error_val_).tolist() + np.stack(error_ts_).tolist() + [tm] + e_ts_persistence_.tolist()
    # Dump data in a .csv
    name = r'/users/terren/solar_forecasting/logs/neural_networks/MO-RNN_v2_v31-1.csv'
    ...
    # 0.1581917709989686 347.11524510383606 [7. 1. 2. 1.] [-4.851724301694098, 5.205891562327206, 1.8233562506125067, 1.8389678292705542, -6.237169844434893]
    theta_  = [-4.851724301694098, 5.205891562327206, 1.8233562506125067, 1.8389678292705542, -6.237169844434893]
    i_cov   = 7
    i_R_lay = 1
    i_M_lay = 2
    R_lay_i = 1
    ...
    X_tr_, Y_tr_, Z_tr_, X_ts_, Y_ts_, Z_ts_ = _split_dataset(X_, Y_, Z_, percentage = 0.8)
    print(X_tr_.shape, Y_tr_.shape, Z_tr_.shape, X_ts_.shape, Y_ts_.shape, Z_ts_.shape)
    # Training and testing Optimal MO-MLP-RNNS Architecture Parameters
    error_ts_, Y_ts_hat_, time_ = _model_training(X_tr_, Y_tr_, X_ts_, Y_ts_, theta_, i_R_lay, i_M_lay, R_lay_i, path = r'/users/terren/solar_forecasting/model/deep_learning/test_{}_'.format(i_init))

    print(error_ts_, error_ts_.mean(), time_, Y_ts_hat_.shape)

    np.savetxt(r'/users/terren/solar_forecasting/model/deep_learning/test_{}_MO-RNN_prediction.csv'.format(i_init), Y_ts_hat_, delimiter = ',')

    x_ = [i_init, time_[0], time_[1], np.mean(error_ts_)] + error_ts_.tolist()

    name = r'/users/terren/solar_forecasting/logs/neural_networks/test_MO-RNN_v31-1.csv'
    with open(name, 'a', newline = '\n') as f:
        writer = csv.writer(f)
        writer.writerow(x_)
