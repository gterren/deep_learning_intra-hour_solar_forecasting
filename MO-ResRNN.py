...
def _MLP_RNN(n_inputs, n_outputs, R_layers_i, rnn_hidden_units_, mlp_hidden_units_, dropout):
    ...
    _output_i = __input_layer(_hidden_i, rnn_hidden_units_, R_layers_i, dropout)
    ...
    if len(index_) > 1:
        ...
        _hidden   = keras.layers.concatenate([_output_i, _hidden_f], axis = 1)

    else:
        #_hidden   = keras.layers.concatenate([_hidden_i, _output_i], axis = 1)
        _hidden   = _output_i
...
def _model_training(X_tr_, Y_tr_, X_ts_, Y_ts_, theta_, i_R_lay, i_M_lay, R_lay_i, path):
    ...
    model_name = path + r'MO-ResRNN_v31-1_{}{}{}{}.h5'.format(i_cov, i_R_lay, i_M_lay, R_lay_i)
...
if validation:
    ...
    # Dump data in a .csv
    name = r'/users/terren/solar_forecasting/logs/neural_networks/MO-ResRNN_v31-1.csv'
    with open(name, 'a', newline = '\n') as f:
        writer = csv.writer(f)
        writer.writerow(x_)
else:
    i_init = int(sys.argv[1])
    # 0.15950534315765635 216.02851223945615 [6. 1. 3. 0.] [-5.778248934354986, 5.402552871419934, 2.9276069993273266, 1.6043684410829018, -7.01754476101349]
    theta_  = [-5.778248934354986, 5.402552871419934, 2.9276069993273266, 1.6043684410829018, -7.01754476101349]
    i_cov   = 6
    i_R_lay = 1
    i_M_lay = 3
    R_lay_i = 0
    ...
    np.savetxt(r'/users/terren/solar_forecasting/model/deep_learning/test_{}_MO-ResRNN_prediction.csv'.format(i_init), Y_ts_hat_, delimiter = ',')

    x_ = [i_init, time_[0], time_[1], np.mean(error_ts_)] + error_ts_.tolist()

    name = r'/users/terren/solar_forecasting/logs/neural_networks/test_MO-ResRNN_v31-1.csv'
    with open(name, 'a', newline = '\n') as f:
        writer = csv.writer(f)
        writer.writerow(x_)
