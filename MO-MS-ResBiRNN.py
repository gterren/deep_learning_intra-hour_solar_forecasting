...
def _MLP_RNN(n_inputs, n_outputs, R_layers_i, R_layers_f, rnn_hidden_units_i_, rnn_hidden_units_f_, mlp_hidden_units_, dropout):
    ...
    _output_f_backward = __input_layer(_hidden_f_backward, rnn_hidden_units_f_, R_layers_f, dropout)
    print(_output_i.shape, _hidden_a.shape, _output_f_forward.shape, _output_f_backward.shape)
    ...
    _output_f_backward = _residuals_layer(_hidden_f_backward, _output_f_backward, dropout)
    # Define The Input Layers for each feature source
    _hidden = keras.layers.concatenate([_output_i, _hidden_a, _output_f_forward, _output_f_backward], axis = 1)
...
def _model_training(X_tr_, Y_tr_, X_ts_, Y_ts_, theta_, i_R_lay, f_R_lay, i_M_lay, R_lay_i, R_lay_f, i_R_neurons, path):
    ...
    model_name = path + r'MO-MS-ResBiRNN_v31-1_{}{}{}{}{}{}.h5'.format(i_cov, i_R_lay, f_R_lay, i_M_lay, R_lay_i, R_lay_f)
...
if validation:
    ...
    print(i_cov, i_R_lay, i_M_lay, R_lay_i, R_lay_f, i_structure)
    ...
    # Dump data in a .csv
    name = r'/users/terren/solar_forecasting/logs/neural_networks/MO-MS-ResBiRNN_v31-1.csv'
    with open(name, 'a', newline = '\n') as f:
        writer = csv.writer(f)
        writer.writerow(x_)
else:
    i_init = int(sys.argv[1])
    # 0.1591227173694667 142.1399686336517 [5. 2. 1. 3. 0. 2.] [-5.11442696891344, 7.4734103245829235, 1.6218073575640384, 2.362149582415101, -8.635350978654234]
    theta_  = [-5.11442696891344, 7.4734103245829235, 1.6218073575640384, 2.362149582415101, -8.635350978654234]
    i_cov   = 5
    i_R_lay = 2
    f_R_lay = 1
    i_M_lay = 3
    ...
    np.savetxt(r'/users/terren/solar_forecasting/model/deep_learning/test_{}_MO-MS-ResBiRNN_prediction.csv'.format(i_init), Y_ts_hat_, delimiter = ',')

    x_ = [i_init, time_[0], time_[1], np.mean(error_ts_)] + error_ts_.tolist()

    name = r'/users/terren/solar_forecasting/logs/neural_networks/test_MO-MS-ResBiRNN_v31-1.csv'
    with open(name, 'a', newline = '\n') as f:
        writer = csv.writer(f)
        writer.writerow(x_)
