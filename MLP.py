...
def _model_training(X_tr_, Y_tr_, X_ts_, Y_ts_, theta_, i_lay, path):
    ...
    model_name = path + r'MLP_v31-1_{}{}.h5'.format(i_cov, i_lay)
...
# BOMLP K-Fold Cross-Validation of the model Parameters
def _get_BOMLP_cross_validation(X_tr_, Y_tr_, i_lay, path):
...
if validation:
    ...
    for i_task in range(Y_tr_.shape[1]):
        ...
        theta_, error_val = _get_BOMLP_cross_validation(X_tr_, Y_tr_[:, i_task][:, np.newaxis], i_lay, path = r'/users/terren/solar_forecasting/model/deep_learning/')
        print(theta_, -error_val)
        # Training Optimal Recurrent MLPs Architecture Parameters
        tm1      = time.time()
        error_ts = _model_training(X_tr_, Y_tr_[:, i_task][:, np.newaxis],
                                   X_ts_, Y_ts_[:, i_task][:, np.newaxis],
                                   theta_, i_lay, path = r'/users/terren/solar_forecasting/model/deep_learning/')[0]
        tm       = time.time() - tm1
        print(tm, error_ts)
    ...
    print(x_)
    # Dump data in a .csv
    name = r'/users/terren/solar_forecasting/logs/neural_networks/MLP_v31-1.csv'
    ...
    #0.16477505225293884 396.58704376220703 [6. 2. 4.] [-3.5936240327248434, 6.481678590284508, 1.0830151189507147, -9.215194715474153, -3.3965927995646434, 7.03850594452014, 1.9661883302572267, -8.151934986163285, -5.203424836673449, 7.224139301584625, 1.2196062850972624, -7.46860875652462, -3.5402980528178225, 5.741426711094045, 2.210079063824385, -5.817189210634567, -5.888486751043206, 6.851722551252331, 1.3661629832617486, -7.5008287859031055, -2.9785298959816724, 7.033916268469314, 2.8978452992431007, -5.960893886762214]
    Theta_     = [[-3.5936240327248434, 6.481678590284508, 1.0830151189507147, -9.215194715474153],
                  [-3.3965927995646434,  7.03850594452014, 1.9661883302572267, -8.151934986163285],
                  [ -5.203424836673449, 7.224139301584625, 1.2196062850972624, -7.468608756524624],
                  [-3.5402980528178225, 5.741426711094045,  2.210079063824385, -5.817189210634567],
                  [ -5.888486751043206, 6.851722551252331, 1.3661629832617486, -7.500828785903106],
                  [-2.9785298959816724, 7.033916268469314, 2.8978452992431007, -5.960893886762214]]
    i_cov = 6
    i_lay = 2
    i_sec = 4
    # Get Experiment for the i-th Job
    ...
    np.savetxt(r'/users/terren/solar_forecasting/model/deep_learning/test_{}_MLP_prediction.csv'.format(i_init), Y_ts_hat_, delimiter = ',')

    x_ = [i_init, time_[0], time_[1], np.mean(error_ts_)] + error_ts_

    name = r'/users/terren/solar_forecasting/logs/neural_networks/test_MLP_v31-1.csv'
    with open(name, 'a', newline = '\n') as f:
        writer = csv.writer(f)
        writer.writerow(x_)
